...
from common_v08 import canonical_json
...
class SQLiteReferenceMonitor:
        ...
        with self._connection() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS effects (
                    ...
                    closed_prefix_digest TEXT NOT NULL,
    ...
    def initialize(
        ...
        *,
        server_cut: int,
        server_time: int,
        trusted_prefix: AuthorityPrefixSnapshot,
    ) -> None:
        trusted_prefix.validate()
        if trusted_prefix.closed_cut != server_cut:
        ...
        with self._connection() as conn:
            ...
            try:
                ...
                conn.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('closed_prefix_digest',?)", (trusted_prefix.closed_prefix_digest,))
                conn.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('authority_source_id',?)", (trusted_prefix.authority_source_id,))
                conn.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('authority_snapshot_digest',?)", (trusted_prefix.snapshot_digest,))
                for cap in caps:
                    conn.execute(
    ...
    @staticmethod
    def _trusted_prefix_from_meta(meta: Mapping[str, str]) -> AuthorityPrefixSnapshot:
        snapshot = AuthorityPrefixSnapshot(
            closed_cut=int(meta["server_cut"]),
            closed_prefix_digest=meta["closed_prefix_digest"],
            authority_source_id=meta["authority_source_id"],
        )
        snapshot.validate()
        if meta.get("authority_snapshot_digest") != snapshot.snapshot_digest:
            raise ValueError("authority snapshot digest mismatch")
        return snapshot
    ...
    def execute(self, request: ControlRequest, *, authority_decision: AuthorityDecision | None = None) -> EffectReceipt:
        ...
        with self._connection() as conn:
            ...
            try:
                ...
                trusted_prefix = self._trusted_prefix_from_meta(meta)
                ...
                try:
                    ...
                    authority_digest = verify_authority_binding(request, descriptor, authority_decision, trusted_prefix)
                ...
                if effect_id is not None:
                    ...
                    conn.execute(
                        """INSERT INTO effects(effect_id,request_id,request_digest,authority_decision_digest,closed_prefix_digest,authority_source_id,descriptor_digest)
                           VALUES(?,?,?,?,?,?,?)""",
                        (
                            effect_id,
                            request.context.request_id,
                            digest,
                            authority_digest,
                            trusted_prefix.closed_prefix_digest,
                            trusted_prefix.authority_source_id,
                ...
                with self._connection() as write_conn:
                    ...
                    try:
                        ...
                        meta = dict(write_conn.execute("SELECT key,value FROM meta").fetchall())
                        trusted_prefix = self._trusted_prefix_from_meta(meta)
                        receipt = EffectReceipt.create(
                            request=request,
                            status="REJECTED",
    ...
    def trusted_prefix(self) -> AuthorityPrefixSnapshot:
        with self._connection() as conn:
            meta = dict(conn.execute("SELECT key,value FROM meta").fetchall())
            return self._trusted_prefix_from_meta(meta)
