...
# ---------------------------------------------------------------------------
# v0.8 authority prefix and atomic cut semantics
# ---------------------------------------------------------------------------
...
@dataclass(frozen=True)
class AuthorityPrefixSnapshot:
    closed_cut: int
    closed_prefix_digest: str
    authority_source_id: str

    def validate(self) -> None:
        require_nonnegative_int(self.closed_cut, "closed_cut")
        require_nonempty_text(self.closed_prefix_digest, "closed_prefix_digest")
        require_nonempty_text(self.authority_source_id, "authority_source_id")
...
class AuthorityCutLedger:
    ...
    def __init__(self, *, start_cut: int = 0, authority_source_id: str = "authority:default") -> None:
        require_nonnegative_int(start_cut, "start_cut")
        require_nonempty_text(authority_source_id, "authority_source_id")
        self._lock = RLock()
        ...
        self._closed_prefix_digest = sha256_digest(("authority-prefix", authority_source_id, start_cut))
    ...
    def authority_source_id(self) -> str:
        return self._authority_source_id

    def prefix_snapshot(self) -> AuthorityPrefixSnapshot:
        with self._lock:
            if not self._closed_cuts:
                raise ValueError("authority prefix has no closed cut")
            return AuthorityPrefixSnapshot(
                closed_cut=self._next_cut - 1,
                closed_prefix_digest=self._closed_prefix_digest,
                authority_source_id=self._authority_source_id,
            )
...
class BranchRepository:
    ...
    semantics are deliberately outside the v0.8 claim surface.
    """
...
class ObligationToken:
    ...
    @property
    def debtor(self) -> str:
        """Compatibility view; normative meaning is explicit in v0.8."""
